...
//	private WorkExperienceService weService;
	...
	@GetMapping("/{username}/link-social-network")
	public ResponseEntity<Response> getSocialNetworkByAccountId(@PathVariable("username") String username) {
		Response response = null;
		try {
			AccountDTO accountDTO = accountService.getLinkSocialNetworkById(username);
			if (accountDTO != null) {
				response = new Response("Get account success", 200, accountDTO);
			} else {
				response = new Response("Account not found", 200, accountDTO);
